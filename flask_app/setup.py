...
def generate_setup_doc_for_file(filename, content, retry_count=3):
    """Generate setup documentation for a single file with retry logic"""
...
{truncated_content}"""
    ...
    for attempt in range(retry_count):
        try:
            current_app.logger.info(f"Generating setup docs for {filename} (attempt {attempt + 1}/{retry_count})")
            ...
            current_app.logger.info(f"Successfully generated setup docs for {filename}")
            return ai_response, None
            
        except Exception as e:
            error_msg = str(e)
            current_app.logger.error(f"Error generating setup docs for {filename} (attempt {attempt + 1}): {error_msg}")
...
def setup_documentation():
    ...
    if not files:
        current_app.logger.warning("No files found in session for /setup_documentation")
    ...
    summary = f"## Setup Documentation Summary\n\n"
...
def regenerate_setup():
    files = session.get('files', [])
    if not files:
        current_app.logger.warning("No files found in session for /api/regenerate_setup")
