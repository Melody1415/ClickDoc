...
def generate_tech_stack_doc_for_file(filename, content, retry_count=3):
    """Generate tech stack documentation for a single file with retry logic"""
...
{truncated_content}"""
    ...
    for attempt in range(retry_count):
        try:
            current_app.logger.info(f"Generating tech stack docs for {filename} (attempt {attempt + 1}/{retry_count})")
            ...
            current_app.logger.info(f"Successfully generated tech stack docs for {filename}")
            return ai_response, None
            
        except Exception as e:
            error_msg = str(e)
            current_app.logger.error(f"Error generating tech stack docs for {filename} (attempt {attempt + 1}): {error_msg}")
...
def tech_stack_documentation():
    ...
    if not files:
        current_app.logger.warning("No files found in session for /tech_stack_documentation")
    ...
    summary = f"## Tech Stack Documentation Summary\n\n"
...
def regenerate_tech_stack():
    files = session.get('files', [])
    if not files:
        current_app.logger.warning("No files found in session for /api/regenerate_tech_stack")
