...
def generate_documentation_for_file(filename, content, retry_count=3):
    """Generate documentation for a single file with retry logic"""
...
{truncated_content}"""
    ...
    for attempt in range(retry_count):
        try:
            current_app.logger.info(f"Generating documentation for {filename} (attempt {attempt + 1}/{retry_count})")
            ...
            current_app.logger.info(f"Successfully generated documentation for {filename}")
            return ai_response, None
            
        except Exception as e:
            error_msg = str(e)
            current_app.logger.error(f"Error generating documentation for {filename} (attempt {attempt + 1}): {error_msg}")
